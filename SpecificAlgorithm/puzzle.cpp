...
int hash(const int state[9],int n) // 将全排列映射为hash值，变相进数 
{
    int count = 0 ,res = 0 ; 
    for ( int i = 0 ; i < n ; i ++)
    {
...
int h( const int board[9] ) //每个数字到其终点所需的步数 
{
    int dis = 0 ; 
    for ( int i = 0 ; i < 9 ; i ++ ) 
    {
    ...
    while ( !open.empty())   //A*Ëã·¨Ö÷Òª²¿·Ö 
    {
        ...
        for ( int i = 0 ; i < 4 ; i ++ ) 
        {
            ...
            {                
                    ...
                    if(color[v] == 1 && (d[u] + 1) < d[v]){//节点在open表中                 
                    	move_[v] = i;                    
                     	f[v] = f[v] - d[v] + d[u] + 1;                   
                      	d[v] = d[u] + 1;                    
                       	parent[v] = u;                                       
                 	...
                 	else if(color[v] == 2 && (d[u]+1)<d[v]){//节点在closed表中                 
                  		move_[v] = i;                    
                    	f[v] = f[v] - d[v] + d[u] + 1;                  
                     	d[v] = d[u] + 1;                    
                      	parent[v] = u;                    
...
bool judge( const int a[9] )  //判断初始状态的逆序数，以确保有解
{
    int count = 0 ; 
    for ( int i = 0 ; i < 9 ; i ++ ) 
    {
